...
يضيف الوكيل ملف `evidence.py` المستوحى من المشاركة الثانية. الأدلة من نوع `OBSERVED` تُسجل فقط بعد تحقق فعلي: وجود الملف على القرص مع بصمة SHA-256، أو نجاح أمر تحقق بخروج يساوي صفرًا. أما المعلومات التي ينتجها النموذج فتظل توجيهًا تخطيطيًا ولا تكفي وحدها لإعلان النجاح. يعيد `all_observed()` حساب SHA-256 للملفات المسجلة عند فحص الاكتمال، ولذلك يكشف تغيير الملف بعد تسجيل الدليل. يُحفظ السجل محليًا في `.nabd/evidence.json` وهو متجاهل في Git افتراضيًا.
